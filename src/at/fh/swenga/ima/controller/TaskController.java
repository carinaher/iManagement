...
public class TaskController {
	...
	public String find(Model model, @RequestParam String searchString, @ModelAttribute("type") String type,
		...
		tasks = taskRepository.findAllTasksForUser("%"+searchString+"%", userDetails.getUsername());
	...
	public String fillData(Model model, @AuthenticationPrincipal UserDetails userDetails,
		...
		LocalDateTime endLocalDateTime = LocalDateTime.from(startDate.toInstant().atZone(ZoneId.of("UTC"))).plusDays(3); // can
																															// only
																															// increment
																															// a
																															// LocalDateTime
		...
		List<TaskModel> tasks = new ArrayList<>();
		tasks.add(new TaskModel("SWENGA", "Software Engineering Advanced", true, startDate,
				df.getDateBetween(startDate, endDate), "FH-Joanneum Graz", userDetails.getUsername()));
		tasks.add(new TaskModel("HVSYS", "Heterogene vernetzte Systeme", true, startDate,
				df.getDateBetween(startDate, endDate), "FH-Joanneum Graz", userDetails.getUsername()));
		tasks.add(new TaskModel("DMT3", "Digitale Medien Technologien 3", false, startDate,
				df.getDateBetween(startDate, endDate), "FH-Joanneum Kapfenberg", userDetails.getUsername()));
		tasks.add(new TaskModel("GPMGT", "Geschäftsprozessmanagement", true, startDate,
				df.getDateBetween(startDate, endDate), "FH-Joanneum Graz", userDetails.getUsername()));
		tasks.add(new TaskModel("TEAMT", "Teamtraining", false, startDate, df.getDateBetween(startDate, endDate),
				"FH-Joanneum Bad Gleichenberg", userDetails.getUsername()));

		List<TaskModel> savedTaskModels = taskRepository.save(tasks);
		savedTaskModels.stream().forEach(t -> t.setUrl("#")); // setter
																// automatically
																// generates a
																// url (with id)
																// to edit this
	...
	public String addTask(@Valid @ModelAttribute TaskModel newTaskModel, BindingResult bindingResult, Model model,
			@AuthenticationPrincipal UserDetails userDetails,
			@RequestParam(value = "returnUrl", required = false) String returnUrl,
		...
		if (task != null) {
			model.addAttribute("errorMessage", "Task already exists!<br>");
				...
				&& !newTaskModel.getUserName().equals(userDetails.getUsername())) {
			...
			if (statusCheckbox == null)
				newTaskModel.setStatus(false);
			else
				newTaskModel.setStatus(true);
			TaskModel savedTaskModel = taskRepository.save(newTaskModel);
			savedTaskModel.setUrl("#"); // setter automatically generates a url
										// (with id) to edit this task
	...
	public String editTask(Model model, @RequestParam int id, @AuthenticationPrincipal UserDetails userDetails,
		...
		if (task == null) {
			...
			return "forward:/calendar";
				...
				&& !task.getUserName().equals(userDetails.getUsername())) {
			// not an admin and not the current user
	...
	public String editStudent(@Valid @ModelAttribute TaskModel editedTaskModel, BindingResult bindingResult,
		...
		if (task == null) {
			...
			model.addAttribute("errorMessage", "Not authorized edit tasks of " + task.getUserName());
				...
				&& !editedTaskModel.getUserName().equals(userDetails.getUsername())) {
			...
			task.setUrl("#"); // setter automatically generates a url to edit
								// this task
